...
// 20/04/2014
//
// Revised by Tim Grocott to correct gaps between thread segments by ensuring a good overlap.
// This is to correct printing issues with resulting STL files.
// 01/07/2019
...
//  sh = segment height (z)
{
	...
	ae = as + s  + (s/1000);    // angle to end of seg (with overlap) <-- Revised by TG
...
//  sh = segment height (z)
{
	...
	//ae = as + s -(s/100);		// angle to end of seg (with overlap)
	ae = as + s +(s/100);		// angle to end of seg (with overlap) <-- Revised by TG
	z = sh*sg;
	pp = p/2;
	//         2,5
	//          /|
